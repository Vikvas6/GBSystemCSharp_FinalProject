...
[RequireComponent(typeof(SolarSystemNetworkManager))]
public class SolarSystemNetworkManagerHUD : NetworkManagerHUD
{
    
    // Runtime variable
    protected bool m_ShowServer2;
            ...
            if (!manager.IsClientConnected() && !NetworkServer.active && manager.matchMaker == null)
            {
                if (noConnection)
                {
                    if (UnityEngine.Application.platform != RuntimePlatform.WebGLPlayer)
                    {
                        ...
                        {
                            ((SolarSystemNetworkManager) manager).playerName = GUI.TextField(new Rect(xpos + 200, ypos, 95, 20), ((SolarSystemNetworkManager) manager).playerName);
                            manager.StartHost();
                        }
                        ypos += spacing;
                    }
